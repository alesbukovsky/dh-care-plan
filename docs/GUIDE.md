...
Paste the patient case text into the case study pane. Highlight parts of it as you work, if needed. It stays pinned on
screen as you scroll and expand sections on the plan side, so you can keep referring back to it while filling out needs,
goals, assessments etc.
...
- **Human needs**: the 8 standard needs, plus 1 specific to Tunxis Dental Hygiene program. This is one of the key
  benefits of using the tool as a typical care plan document represents each need across several separate sections (e.g.
  assessment, prioritization, planning). Here you enter each need once, and the tool spreads its parts into those
...
No data is sent anywhere outside of your computer. Everything is saved automatically to your browser's local storage a
second or so after each edit, and it's still there next time you open the app, even after closing the browser or
...
indicator at the bottom of the command bar, next to the app version. Your work stays in memory for that session only, so
