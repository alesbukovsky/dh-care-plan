...
The `dhplan schema <type>` command SHALL accept exactly one positional argument, one of `plan`, `template`, or
`mapping`, and print the corresponding schema's JSON Schema document to stdout. The command SHALL obtain that document
by calling `getPlanSchema()` / `getTemplateSchema()` / `getMappingSchema()` rather than calling `z.toJSONSchema` on the
zod schema directly.
...
`getMappingSample()`, and SHALL NOT print the JSON Schema document.
...
- **THEN** the system SHALL print the JSON Schema document returned by `getMappingSchema()` to stdout

#### Scenario: Requesting an invalid schema type via `schema`

- **WHEN** a user runs `dhplan schema <anything other than plan, template, or mapping>`
- **THEN** the system SHALL print an error identifying the valid schema types (`plan`, `template`, `mapping`) and exit
...
- **THEN** the system SHALL print the example JSON document returned by `getMappingSample()` to stdout, and SHALL NOT
  print a JSON Schema document
...
`template`, or `mapping`, and `file`, a path to the file to validate. It SHALL validate `file` against the schema named
by `type` (using `validateData` for `plan`, `validateTemplate` for `template`, or `validateMapping` for `mapping`) and
report the result.
...
- **WHEN** a user runs `dhplan validate mapping <file>` and `<file>` contains JSON that satisfies `Mapping` (every key
  present)
- **THEN** the system SHALL print a success message to stdout and exit `0`

#### Scenario: Validating an invalid mapping file

- **WHEN** a user runs `dhplan validate mapping <file>` and `<file>` contains JSON that violates `Mapping` (including an
  incomplete mapping missing a key), or content that is not valid JSON
- **THEN** the system SHALL print every discovered validation issue to stderr and exit non-zero, without printing a
  success message

#### Scenario: Requesting an invalid schema type via `validate`

- **WHEN** a user runs `dhplan validate <anything other than plan, template, or mapping> <file>`
- **THEN** the system SHALL print an error identifying the valid schema types (`plan`, `template`, `mapping`) and exit
...
The command SHALL also accept an optional `--mapping <file>` option. When given, the command SHALL read `<file>` from
disk and pass its contents as `render()`'s third argument; when omitted, `render()` is called with no third argument
(using its built-in default mapping).
...
- **WHEN** a user runs `dhplan render <plan> <template> <output> --mapping <mapping-file>` where `plan` and `template`
  are valid and `<mapping-file>` contains JSON satisfying `Mapping` (every key present)
- **THEN** the system SHALL render using that mapping and write the rendered document to `output`

#### Scenario: Rendering with an invalid `--mapping` file

- **WHEN** a user runs `dhplan render <plan> <template> <output> --mapping <mapping-file>` where `<mapping-file>` fails
  `Mapping` validation (including an incomplete mapping missing a key), or cannot be read as JSON
- **THEN** the system SHALL print every discovered issue to stderr and exit non-zero, without writing `output`
...
The command SHALL also accept an optional `--mapping <file>` option. When given, the command SHALL read and validate
`<file>` against `Mapping` (every key required) before passing it to `buildTemplateData`; when omitted,
`buildTemplateData` is called with no mapping argument (using its built-in default).
...
- **WHEN** a user runs `dhplan inspect <plan> --mapping <mapping-file>` where `plan` is valid and `<mapping-file>`
  contains JSON satisfying `Mapping` (every key present)
- **THEN** the system SHALL print `Template` JSON built using that mapping

#### Scenario: Inspecting with an invalid `--mapping` file

- **WHEN** a user runs `dhplan inspect <plan> --mapping <mapping-file>` where `<mapping-file>` fails `Mapping`
  validation (including an incomplete mapping missing a key), or cannot be read as JSON
