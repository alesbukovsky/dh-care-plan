...
the corresponding schema's JSON Schema document to stdout. The command SHALL obtain that document by calling
`getPlanSchema()` / `getTemplateSchema()` rather than calling `z.toJSONSchema` on the zod schema directly.

The command SHALL also accept an optional `--sample` flag. When `--sample` is passed, the command SHALL instead print an
example JSON document matching `<type>`'s shape, obtained by calling `getPlanSample()` / `getTemplateSample()`, and
SHALL NOT print the JSON Schema document.
