...
`plan.needs` into `Template`'s `patient`, `appointments`, `assessments`, and `statements`:
...
- Each statement's `outcome` SHALL be derived from the source `Need`'s `outcome`: `outcome.note` SHALL be copied 1:1,
  and `outcome.status` SHALL be mapped to `outcome.label` as follows: `"met"` -> `"Met"`, `"partial"` ->
...
#### Scenario: Mapping outcome status to a display label

- **WHEN** `buildTemplateData` maps an unmet `Need` whose `outcome.status` is `"met"`, `"partial"`, or `"unmet"`
- **THEN** the resulting `Statement`'s `outcome.label` SHALL be `"Met"`, `"Partially met"`, or `"Not met"` respectively,
  and `outcome.note` SHALL equal the `Need`'s `outcome.note`
