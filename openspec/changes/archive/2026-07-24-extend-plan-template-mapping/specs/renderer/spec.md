...
`appointments`, `assessments`, and `statements`:
...
- Each statement's `outcome` SHALL be derived from the source `Need`'s
  ...
  SHALL be mapped to `outcome.label` as follows: `"met"` -> `"Met"`,
  `"partial"` -> `"Partially met"`, `"unmet"` -> `"Not met"`.
...
- **WHEN** `buildTemplateData` maps an unmet `Need` whose `outcome.status`
  is `"met"`, `"partial"`, or `"unmet"`
- **THEN** the resulting `Statement`'s `outcome.label` SHALL be `"Met"`,
  `"Partially met"`, or `"Not met"` respectively, and `outcome.note` SHALL
  equal the `Need`'s `outcome.note`
