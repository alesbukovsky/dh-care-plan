...
validates it against `Plan` using zod's `safeParse`, returning every
...
- **WHEN** `validateData` is given JSON content that violates `Plan`
...
- **WHEN** `validateTemplate` is given a `.docx` file whose docxtemplater
  ...
  `Template` defines at that scope
...
- **WHEN** `validateTemplate` is given a `.docx` file that does not
  reference every tag `Template` defines
