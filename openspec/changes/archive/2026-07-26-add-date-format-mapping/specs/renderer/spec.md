...
- **WHEN** `buildTemplateData` maps a `Plan` whose `patient.dob` is
...
- **WHEN** `buildTemplateData` maps a `Plan`'s `appointments` list using
...
- **WHEN** `buildTemplateData` maps a `Goal` whose `doneBy` is
  `undefined`
...
- **WHEN** `buildTemplateData` is called without an explicit `Config`
...
- **WHEN** `buildTemplateData` is called with an explicit `Config` whose
