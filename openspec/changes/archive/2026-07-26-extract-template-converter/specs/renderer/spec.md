...
**Reason**: `convertData` (and its mapping/formatting behavior)
moved out of `renderer.ts` into its own `converter` capability
(`packages/core/src/converter.ts`), since it is pure `Plan`→`Template`
mapping logic independent of `render()`'s file-I/O-adjacent
orchestration.

**Migration**: See the `converter` capability's "`convertData`
converts a Plan into flattened Template data" requirement, which carries
this requirement's behavior and scenarios forward unchanged.
...
(`convertData`, `packages/core/src/renderer.ts`) that maps `Plan`'s
top-level fields and `plan.needs` into `Template`'s `patient`,
`appointments`, `assessments`, and `statements`. `convertData`
SHALL accept an optional resolved `Config` as its second parameter,
...
- Each statement's `goals` SHALL contain one entry per `Goal` in the
  ...
  `doneBy` formatted using the given `Config`'s `format.date` pattern
  when present (left `undefined` when the source `Goal` has no
  `doneBy`), plus a generated `label` of the form `<n><letter>` where
...
- **WHEN** `render()` has a validated `Plan` with a mix of met and unmet
  needs and calls `convertData` with it
...
- **WHEN** `convertData` maps a `Goal` whose `doneBy` is
  `undefined`
- **THEN** the resulting `Statement` goal's `doneBy` SHALL remain
  `undefined`, without attempting to format it
...
Once both the plan and template are valid, `render()` SHALL obtain
template data by calling `convertData` (the `converter` capability,
`packages/core/src/converter.ts`), then construct a docxtemplater
instance from the template buffer (via the existing `createTemplater`
helper), render it using that template data, and return the resulting
document bytes. `render()` SHALL accept an optional third `ArrayBuffer`
parameter carrying a config file's raw contents; if provided, `render()`
SHALL validate it against `Config` (every key required) before passing it
to `convertData`. If omitted, `render()` SHALL use `DEFAULT_CONFIG`.
